...
func (o *BuildOptions) AddFlags(cmd *cobra.Command, imageBuilderRepo string) {
	...
	StringMapVarWithViper(cmd, &o.AdditionalMetadata, viperBuildPrefix, "additional-metadata", nil, "Add any additional metadata to tag the image with.")
	...
	// GPU
	BoolVarWithViper(cmd, &o.AddGpuSupport, viperGpuPrefix, "enable-gpu-support", false, "This will configure GPU support in the image")
	StringVarWithViper(cmd, &o.GpuVendor, viperGpuPrefix, "gpu-vendor", "", "The architecture of the GPU (currently supported: nvidia, amd)")
	// AMD
	StringVarWithViper(cmd, &o.AMDVersion, viperGpuPrefix, "amd-driver-version", "6.0.2", "The AMD driver version")
	StringVarWithViper(cmd, &o.AMDDebVersion, viperGpuPrefix, "amd-deb-version", "6.0.60002-1", "The AMD deb version")
	StringVarWithViper(cmd, &o.AMDUseCase, viperGpuPrefix, "amd-usecase", "dkms", "A comma-dliminated string of usecases for the AMDGPU installer")
	// NVIDIA flags
	StringVarWithViper(cmd, &o.NvidiaVersion, viperGpuPrefix, "nvidia-driver-version", "525.129.03", "The NVIDIA driver version")
	StringVarWithViper(cmd, &o.NvidiaBucket, viperGpuPrefix, "nvidia-bucket", "", "The bucket name in which the NVIDIA components are stored")
	StringVarWithViper(cmd, &o.NvidiaInstallerLocation, viperGpuPrefix, "nvidia-installer-location", "", "The NVIDIA installer location in the bucket - this must be acquired from NVIDIA and uploaded to your bucket")
	StringVarWithViper(cmd, &o.NvidiaTOKLocation, viperGpuPrefix, "nvidia-tok-location", "", "The NVIDIA .tok file location in the bucket - this must be acquired from NVIDIA and uploaded to your bucket")
	IntVarWithViper(cmd, &o.NvidiaGriddFeatureType, viperGpuPrefix, "nvidia-gridd-feature-type", -1, "The gridd feature type - See https://docs.nvidia.com/license-system/latest/nvidia-license-system-quick-start-guide/index.html#configuring-nls-licensed-client-on-linux for more information")
